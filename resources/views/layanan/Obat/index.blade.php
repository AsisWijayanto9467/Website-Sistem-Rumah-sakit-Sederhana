    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Data Obat</h1>
                    <p class="text-gray-600 mt-1">Kelola data obat-obatan yang tersedia di sistem</p>
                </div>
            </div>
        </div>
                                ...
                                <div class="flex gap-2">
                                    <div class="relative flex-1">
                                        ...
                                        <input type="text" 
                                            ...
                                            name="search"
                                            value="{{ $search }}"
                                            placeholder="Cari berdasarkan nama, harga, deskripsi, atau stok..." 
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-2">
                                    <div class="flex items-start">
                                        <span class="text-xs font-medium text-gray-700 mr-2">Harga:</span>
                                        <span class="text-sm text-gray-800 font-medium">Rp {{ number_format($obat->harga, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="flex items-start">
                                        <span class="text-xs font-medium text-gray-700 mr-2">Stok:</span>
                                        <span class="text-sm text-gray-800">{{ $obat->stock }} unit</span>
                                    </div>
                                </div>
                            </td>
                                ...
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-gray-600 mb-6">
                                        @if($search)
                                            ...
                                            <a href="{{ route('medication.index') }}" class="text-blue-600 hover:underline">reset pencarian</a>
                                        @else
            ...
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div class="text-sm text-gray-700">
                        Menampilkan 
                        <span class="font-medium">{{ $medications->firstItem() }}</span>
                        sampai 
                        <span class="font-medium">{{ $medications->lastItem() }}</span>
                        dari 
                        <span class="font-medium">{{ $medications->total() }}</span>
                        data obat
                    </div>
                    ...
                    <div class="flex items-center space-x-1">
                        ...
                        @else
                        <a href="{{ $medications->previousPageUrl() }}{{ $search ? '&search=' . $search : '' }}{{ request('per_page') ? '&per_page=' . request('per_page') : '' }}" 
                        ...
                        <!-- Page Numbers -->
                        @foreach(range(1, $medications->lastPage()) as $page)
                            ...
                            @elseif($page >= $medications->currentPage() - 2 && $page <= $medications->currentPage() + 2)
                            <a href="{{ $medications->url($page) }}{{ $search ? '&search=' . $search : '' }}{{ request('per_page') ? '&per_page=' . request('per_page') : '' }}" 
                               class="px-3 py-1.5 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                                {{ $page }}
                            </a>
                            @endif
                        ...
                        <a href="{{ $medications->nextPageUrl() }}{{ $search ? '&search=' . $search : '' }}{{ request('per_page') ? '&per_page=' . request('per_page') : '' }}" 
                           class="px-3 py-1.5 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
        ...
        <form id="delete-form-{{ $obat->id }}" 
              action="{{ route('medication.destroy', $obat->id) }}" 
              method="POST" 
              class="hidden">
            @csrf
            @method('DELETE')
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            window.confirmDelete = function(obatId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const form = document.getElementById(`delete-form-${obatId}`);
                        if (form) {
                            form.submit();
                        }
                    }
            ...
            const stockBadges = document.querySelectorAll('[class*="bg-yellow-100"], [class*="bg-red-100"]');
            stockBadges.forEach(badge => {
                const row = badge.closest('tr');
                if (row && badge.classList.contains('bg-red-100')) {
                    row.classList.add('bg-red-50');
                } else if (row && badge.classList.contains('bg-yellow-100')) {
